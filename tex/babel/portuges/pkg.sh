...
# adding the Babel portuges to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/portuges; then
		pkg_remove latex/portuges
	fi
	pkg_remove_rcp_obsolete latex/portuges
	return 0 # bash errors on empty function...
}
...
VERSION: 1.2t 2021-07-09
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for portuges for the LaTeX engine
KXPATH:
	tex babel/portuges
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/portuges.zip
END:
END_CID
