...
# adding the Babel danish to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/danish; then
		pkg_remove latex/danish
	fi
	pkg_remove_rcp_obsolete latex/danish
	return 0 # bash errors on empty function...
}
...
VERSION: 1.3s 2021-02-06
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for danish for the LaTeX engine
KXPATH:
	tex babel/danish
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/danish.zip
END:
END_CID
