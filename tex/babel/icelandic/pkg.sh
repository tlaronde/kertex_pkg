...
# adding the Babel icelandic to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/icelandic; then
		pkg_remove latex/icelandic
	fi
	pkg_remove_rcp_obsolete latex/icelandic
	return 0 # bash errors on empty function...
}
...
VERSION: 1.3 2019-07-04
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for icelandic for the LaTeX engine
KXPATH:
	tex babel/icelandic
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/icelandic.zip
END:
END_CID
