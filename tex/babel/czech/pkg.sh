...
# adding the Babel czech to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/czech; then
		pkg_remove latex/czech
	fi
	pkg_remove_rcp_obsolete latex/czech
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for czech for the LaTeX engine
KXPATH:
	tex babel/czech
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/czech.zip
END:
END_CID
