...
# adding the Babel piedmontese to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/piedmontese; then
		pkg_remove latex/piedmontese
	fi
	pkg_remove_rcp_obsolete latex/piedmontese
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for piedmontese for the LaTeX engine
KXPATH:
	tex babel/piedmontese
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/piedmontese.zip
END:
END_CID
