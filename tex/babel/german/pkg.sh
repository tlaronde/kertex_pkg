...
# adding the Babel german to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/german; then
		pkg_remove latex/german
	fi
	pkg_remove_rcp_obsolete latex/german
	pkg_remove_rcp_obsolete latex/./german
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for german for the LaTeX engine
KXPATH:
	tex babel/german
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/german.zip
END:
END_CID
