...
# adding the Babel thai to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/thai; then
		pkg_remove latex/thai
	fi
	pkg_remove_rcp_obsolete latex/thai
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for thai for the LaTeX engine
KXPATH:
	tex babel/thai
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/thai.zip
END:
END_CID
