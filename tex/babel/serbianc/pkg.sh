...
# adding the Babel serbianc to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/serbianc; then
		pkg_remove latex/serbianc
	fi
	pkg_remove_rcp_obsolete latex/serbianc
	return 0 # bash errors on empty function...
}
...
VERSION: 3.2a 2022-09-30
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for serbianc for the LaTeX engine
KXPATH:
	tex babel/serbianc
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/serbianc.zip
END:
END_CID
