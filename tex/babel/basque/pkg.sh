...
# adding the Babel basque to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/basque; then
		pkg_remove latex/basque
	fi
	pkg_remove_rcp_obsolete latex/basque
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for basque for the LaTeX engine
KXPATH:
	tex babel/basque
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/basque.zip
END:
END_CID
