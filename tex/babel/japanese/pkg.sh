...
# adding the Babel japanese to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/japanese; then
		pkg_remove latex/japanese
	fi
	pkg_remove_rcp_obsolete latex/japanese
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for japanese for the LaTeX engine
KXPATH:
	tex babel/japanese
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/japanese.zip
END:
END_CID
