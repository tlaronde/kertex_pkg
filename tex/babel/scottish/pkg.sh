...
# adding the Babel scottish to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/scottish; then
		pkg_remove latex/scottish
	fi
	pkg_remove_rcp_obsolete latex/scottish
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for scottish for the LaTeX engine
KXPATH:
	tex babel/scottish
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/scottish.zip
END:
END_CID
