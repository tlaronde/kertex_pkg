...
# adding the Babel malay to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/malay; then
		pkg_remove latex/malay
	fi
	pkg_remove_rcp_obsolete latex/malay
	return 0 # bash errors on empty function...
}
...
BEGIN_CID
NAME: tex/babel/malay
VERSION: 1.0m
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for malay for the LaTeX engine
KXPATH:
	tex babel/malay
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/malay.zip
END:
END_CID
