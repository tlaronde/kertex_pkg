...
# adding the Babel french to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/french; then
		pkg_remove latex/french
	fi
	pkg_remove_rcp_obsolete latex/french
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for french for the LaTeX engine
NOTES:
...
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/french.zip
	GET /macros/latex/contrib/carlisle/scalefnt.sty
END:
END_CID
