...
# adding the Babel irish to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/irish; then
		pkg_remove latex/irish
	fi
	pkg_remove_rcp_obsolete latex/irish
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for irish for the LaTeX engine
KXPATH:
	tex babel/irish
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/irish.zip
END:
END_CID
