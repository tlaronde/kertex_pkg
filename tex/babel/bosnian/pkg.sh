...
# adding the Babel bosnian to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/bosnian; then
		pkg_remove latex/bosnian
	fi
	pkg_remove_rcp_obsolete latex/bosnian
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for bosnian for the LaTeX engine
KXPATH:
	tex babel/bosnian
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/bosnian.zip
END:
END_CID
