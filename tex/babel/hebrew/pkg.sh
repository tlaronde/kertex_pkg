...
# adding the Babel hebrew to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/hebrew; then
		pkg_remove latex/hebrew
	fi
	pkg_remove_rcp_obsolete latex/hebrew
	return 0 # bash errors on empty function...
}
...
VERSION: 2.4a 2023-08-22
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for hebrew for the LaTeX engine
KXPATH:
	tex babel/hebrew
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/hebrew.zip
END:
END_CID
