...
# adding the Babel romanian to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/romanian; then
		pkg_remove latex/romanian
	fi
	pkg_remove_rcp_obsolete latex/romanian
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for romanian for the LaTeX engine
KXPATH:
	tex babel/romanian
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/romanian.zip
END:
END_CID
