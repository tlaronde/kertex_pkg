...
# adding the Babel interlingua to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/interlingua; then
		pkg_remove latex/interlingua
	fi
	pkg_remove_rcp_obsolete latex/interlingua
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for interlingua for the LaTeX engine
KXPATH:
	tex babel/interlingua
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/interlingua.zip
END:
END_CID
