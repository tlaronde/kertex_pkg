...
# adding the Babel breton to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/breton; then
		pkg_remove latex/breton
	fi
	pkg_remove_rcp_obsolete latex/breton
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for breton for the LaTeX engine
KXPATH:
	tex babel/breton
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/breton.zip
END:
END_CID
