...
# adding the Babel latvian to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/latvian; then
		pkg_remove latex/latvian
	fi
	pkg_remove_rcp_obsolete latex/latvian
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for latvian for the LaTeX engine
KXPATH:
	tex babel/latvian
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/latvian.zip
END:
END_CID
