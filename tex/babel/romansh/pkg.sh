...
# adding the Babel romansh to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/romansh; then
		pkg_remove latex/romansh
	fi
	pkg_remove_rcp_obsolete latex/romansh
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for romansh for the LaTeX engine
KXPATH:
	tex babel/romansh
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/romansh.zip
END:
END_CID
