...
# adding the Babel sorbian to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/sorbian; then
		pkg_remove latex/sorbian
	fi
	pkg_remove_rcp_obsolete latex/sorbian
	return 0 # bash errors on empty function...
}
...
VERSION: 1.0j 2021-11-06
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for sorbian for the LaTeX engine
KXPATH:
	tex babel/sorbian
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/sorbian.zip
END:
END_CID
