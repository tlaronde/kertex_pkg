...
# adding the Babel greek to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/greek; then
		pkg_remove latex/greek
	fi
	pkg_remove_rcp_obsolete latex/greek
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for greek for the LaTeX engine
KXPATH:
	tex babel/greek
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/greek.zip
END:
END_CID
