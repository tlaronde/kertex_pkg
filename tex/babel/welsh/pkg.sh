...
# adding the Babel welsh to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/welsh; then
		pkg_remove latex/welsh
	fi
	pkg_remove_rcp_obsolete latex/welsh
	return 0 # bash errors on empty function...
}
...
DESCRIPTION:
	Babel contrib for welsh for the LaTeX engine
KXPATH:
	tex babel/welsh
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/welsh.zip
END:
END_CID
