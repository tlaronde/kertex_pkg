...
# adding the Babel indonesian to the kerTeX installation.
#
...
pkg_post_apply()
{
	if pkg_is_installed latex/indonesian; then
		pkg_remove latex/indonesian
	fi
	pkg_remove_rcp_obsolete latex/indonesian
	return 0 # bash errors on empty function...
}
...
BEGIN_CID
NAME: tex/babel/indonesian
VERSION: 1.0m
LICENCE: The LaTeX Project Public License 1.3
KERTEX_VERSION: 0.99.22.0
DESCRIPTION:
	Babel contrib for indonesian for the LaTeX engine
KXPATH:
	tex babel/indonesian
SOURCES:
	LCD HOME/..
	GET /macros/latex/contrib/babel-contrib/indonesian.zip
END:
END_CID
